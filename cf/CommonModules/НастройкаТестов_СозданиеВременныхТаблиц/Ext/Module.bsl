...
Процедура ДополнитьДопТаблицы(НастройкаТеста, ДопТаблицы, НомераСессий = Неопределено)
	...
	//ДопТаблицы.Вставить("ПодготовленныеОператоры", 			ДанныеРегистраДляИзменения(НастройкаТеста, "ПодготовленныеОператоры", НомераСессий));
	//ДопТаблицы.Вставить("ПараметрыПодготовленныхОператоров", 	ДанныеРегистраДляИзменения(НастройкаТеста, "ПараметрыПодготовленныхОператоров", НомераСессий));
	...
	//ДопТаблицы.Вставить("НаполненияВременныхТаблицPostgres", НастройкаТестовСервер.ПустаяТаблицаРегистраСведений("НаполненияВременныхТаблицPostgres")); // Основная таблица
	//НастройкаТестовСервер.ДобавитьКолонкиИзмененияВТаблицу(ДопТаблицы.НаполненияВременныхТаблицPostgres);
	//ДопТаблицы.Вставить("ТекущееНаполненияВТ", 					ПрошлоеНаполнениеВТ(НастройкаТеста)); // Таблица для обрабоки порций, которые потом будут скидываться в основную таблицу. 
	//ДопТаблицы.Вставить("НачалоТекущегоНаполненияВТ",			ПрошлоеНачалоТекущегоНаполненияВТ(НастройкаТеста, ДопТаблицы.ТекущееНаполненияВТ)); // Данные по текущим наполнениям ВТ - событиям создания ВТ или очистки ВТ
	//ДопТаблицы.НачалоТекущегоНаполненияВТ.Индексы.Добавить("НомерСессии,ИмяТаблицы");
...
Процедура ВыполнитьОдинПоток(НастройкаТеста, ТаблицаРезультатов = Неопределено, ДопТаблицы) Экспорт	
	...
	//ЗафиксироватьНаполнениеВТПриЗавершении(НастройкаТеста, ДопТаблицы, ЭтапОбработки.Завершен);	
	...
	//СохранитьВременнуюТаблицы(НастройкаТеста, ДопТаблицы.ПодготовленныеОператоры, 			"ПодготовленныеОператоры", ДопТаблицы.Многопоточность);
	//СохранитьВременнуюТаблицы(НастройкаТеста, ДопТаблицы.ПараметрыПодготовленныхОператоров, 	"ПараметрыПодготовленныхОператоров", ДопТаблицы.Многопоточность); 
	...
	//СохранитьВременнуюТаблицы(НастройкаТеста, ДопТаблицы.НаполненияВременныхТаблицPostgres, 	"НаполненияВременныхТаблицPostgres", Истина);
...
Функция ИсправлениеОчисткиВременнойТаблицы(НастройкаТеста, СтрокаТаблицы, Знач ТекстЗапроса, ДопТаблицы) Экспорт 
	...
	//СтрЗаменить(СтрокаТаблицы.ТекстЗапросаMSSQL, "#tt", "tt");
	...
	//НастройкаТестовСервер.ДобавитьПроизвольнуюИнформацию(СтрокаТаблицы, СтрШаблон("ИмяВТ = %1", ИмяТаблицы));
	
	ОбработатьСозданиеОчисткуВТ(НастройкаТеста, СтрокаТаблицы, ИмяТаблицы, ДопТаблицы);
...
Функция СозданиеИндексаВременнойТаблицы(НастройкаТеста, СтрокаТаблицы, Знач ТекстЗапроса, ДопТаблицы) Экспорт 
	...
	Иначе           
		...
		//СтрокаТаблицы.ТекстОшибки = "Не удалось определить вид индекса ВТ";
	...
	ДополнитьНаполнениеВТ(НастройкаТеста, СтрокаТаблицы, СтрЗаменить(ИмяВТ, "pg_temp.", ""), ДопТаблицы); 
...
Функция СоответствиеИменТиповДанных() Экспорт 
	...
	//НоваяСтрока = СоответствиеИменТиповДанных.Добавить();
	//НоваяСтрока.ИмяТипаMSSQL = "tinyint";
	//НоваяСтрока.ИмяТипаPG = "SMALLINT";
	
	//НоваяСтрока = СоответствиеИменТиповДанных.Добавить();
	//НоваяСтрока.ИмяТипаMSSQL = "uniqueidentifier";
	//НоваяСтрока.ИмяТипаPG = "CHAR";
...
Функция ИсправитьБинари1ВТекстеПриЗаполненииВТ(НастройкаТеста, СтрокаТаблицы, Знач Результат, ДопТаблицы) Экспорт 
				...
				Для каждого ПодстрокаПоиска Из ПолеИсточник.МассивПолей Цикл
					...
					Если СтрНайтиПоРегулярномуВыражению(ПодстрокаПоиска, НастройкаТестовКлиентСервер.Шаблон0x01вCase()).Длина > 0 Тогда
						...
						Если Прав(СтрокаТаблицы.ПланЗапроса.ПоляПриемника[Инд], СтрДлина("_type")) = "_type" Тогда
							ПодстрокаЗамены = СтрЗаменитьПоРегулярномуВыражению(ПодстрокаПоиска, НастройкаТестовКлиентСервер.Шаблон0x01вCase(), "$1'\\\\001'::BYTEA");
						Иначе
	...
	Если ЕстьОтдельноСтроящие0x01 Тогда 
	...
	ИначеЕсли ЕстьВValues0x01 Тогда
	...
	Иначе
		...
		Если ОбновлятьДанныеВРегистрах Тогда
			...
			Если ДопТаблицы <> Неопределено Тогда
				...
				//СтрокиТаблицы = ДопТаблицы.ПоляВременныхТаблицыPostgres.НайтиСтроки(Новый Структура("НастройкаТеста,НомерСессии,ИмяТаблицы,ДатаСоздания",НастройкаТеста,НомерСессии,ИмяТаблицы, ДатаСоздания));
		...
		Если Не Рез и ДопТаблицы <> Неопределено Тогда
			...
			// Нафига исключение, просто добавлю ошибку
			//ВызватьИсключение СтрШаблон("Ошибка при заполнении полей временной таблицы %1 Postgres в соединении %2, номер события %3", ИмяТаблицы, НомерСессии, НомерСобытия);
			//НастройкаТестовСервер.ДобавитьПроизвольнуюИнформацию(СтрокаТаблицы, СтрШаблон("ИмяНеСозданнойВТ = %1", ИмяТаблицы));
...
Процедура ПроверкаСуществованияВременнойТаблицы(НастройкаТеста, СтрокаТаблицы, ТаблицаРезультатов, ДопТаблицы) Экспорт 
	...
	//НастройкаТестовСервер.ДобавитьПроизвольнуюИнформацию(СтрокаТаблицы, СтрШаблон("ИмяВТ = %1", ИмяТаблицы), Истина);
	...
	//ИначеЕсли СтрокиВТ.Количество() = 1 И СтрокиВТ[0].ОшибкаСоздания Тогда
	ИначеЕсли СтрокиВТ.Количество() > 0 И СтрокиВТ[СтрокиВТ.ВГраница()].ОшибкаСоздания Тогда
		...
		Если ДанныеПолейВременнойТаблицы.Количество() > 0 Тогда
			...
			// Переделал с первой строки на последнюю
			//СтрокаВТ = СтрокиВТ[0];
	...
	ДополнитьНаполнениеВТ(НастройкаТеста, СтрокаТаблицы, ИмяТаблицы, ДопТаблицы);
...
Функция СохранитьРегистрНастройкиТеста(НастройкаТеста, ТаблицаРегистра, ИмяРегистра)
...
Процедура УстановитьИмяТипа(НоваяСтрока)
	...
	// Откидываем данные в скобках например numeric(3, 0) --> numeric
	//Разделители = "( ";
	//
	//Для Сч = 1 По СтрДлина(НоваяСтрока.Тип) Цикл
	//	Если СтрНайти(Разделители, Сред(НоваяСтрока.Тип, Сч, 1)) > 0 Тогда
	//		НоваяСтрока.ИмяТипа = Лев(НоваяСтрока.Тип, Сч - 1);	
	//	КонецЕсли;	
	//КонецЦикла;
	//
	//Если ПустаяСтрока(НоваяСтрока.ИмяТипа) Тогда
	//	НоваяСтрока.ИмяТипа	= НоваяСтрока.Тип;
	//КонецЕсли;
...
// При завершении этапа перекинем накопленные наполнения ВТ из ТЗ ТекущееНаполненияВТ В НаполненияВременныхТаблицPostgres
Процедура ЗафиксироватьНаполнениеВТПриЗавершении(НастройкаТеста, ДопТаблицы, ЭтапЗавершен)
	
	// Выберем только имеющиеся данные в ТекущееНаполненияВТ
	Копия = ДопТаблицы.ТекущееНаполненияВТ.Скопировать(, "НомерСессии,ИмяТаблицы");
	Копия.Свернуть("НомерСессии,ИмяТаблицы");
	Для каждого СтрокаКопии Из Копия Цикл
		
		СтрокиНачала = ДопТаблицы.НачалоТекущегоНаполненияВТ.НайтиСтроки(Новый Структура("НомерСессии,ИмяТаблицы", СтрокаКопии.НомерСессии, СтрокаКопии.ИмяТаблицы));
		Если ЗначениеЗаполнено(СтрокиНачала) Тогда
			
			СтрокаНачала = СтрокиНачала[0];		
			// Если Этап не завершен, то для текущих наполнений  будем заполнять период = 01.01.2000 - это будет признак при следующем запуске, что эти наполнения не зафиксированы до конца
			Если Не ЭтапЗавершен Тогда
				СтрокаНачала.Период = '20000101';	
			КонецЕсли;
			ЗаполнитьПоляНачалаИПеренестиНаполнениеВТ(СтрокаНачала, , ДопТаблицы);	
			
		// Если опять нарвались на инсерты без предварительного создания или опустошения ВТ, то проставим ошибки
		Иначе 
		
			СтруктураПоиска = Новый Структура("НомерСессии,ИмяТаблицы");
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаКопии);
			СтрокиТекНаполнения = ДопТаблицы.ТекущееНаполненияВТ.НайтиСтроки(СтруктураПоиска);
			УстановитьПризнакПропущеноНаполениеВТ(НастройкаТеста,СтрокиТекНаполнения, ДопТаблицы);
...
// Текущее события либо создание ВТ либо очистка ВТ. Скидывает накопленные данные по наполнению ВТ из ТЗ ТекущееНаполненияВТ В НаполненияВременныхТаблицPostgres
Процедура ОбработатьСозданиеОчисткуВТ(НастройкаТеста, СтрокаТаблицы, ИмяТаблицы, ДопТаблицы)
	
	//СтруктураПоиска = Новый Структура("НомерСессии,ИмяТаблицы");
	//СтруктураПоиска.НомерСессии = СтрокаТаблицы.НомерСессии;
	//СтруктураПоиска.ИмяТаблицы = ИмяТаблицы;
	//// Ищем данные по началу текущего наполнения
	//СтрокиНачала = ДопТаблицы.НачалоТекущегоНаполненияВТ.НайтиСтроки(СтруктураПоиска);
	//СтрокиТекНаполнения = ДопТаблицы.ТекущееНаполненияВТ.НайтиСтроки(СтруктураПоиска);
	//
	//// Если строк не накопилось, то это начало логов 
	//Если СтрокиНачала.Количество() = 0 Тогда
	//	
	//	ДанныеНачала = ДопТаблицы.НачалоТекущегоНаполненияВТ.Добавить();
	//	ДанныеНачала.НомерСессии = СтрокаТаблицы.НомерСессии;
	//	ДанныеНачала.ИмяТаблицы = ИмяТаблицы;
	//	
	//	// Установим признак непрезентабельности данных в соответствующих строках ТаблицаРезультатов
	//	УстановитьПризнакПропущеноНаполениеВТ(НастройкаТеста, СтрокиТекНаполнения, ДопТаблицы)
	//	
	//Иначе
	//	
	//	ДанныеНачала = СтрокиНачала[0];	
	//	
	//	// Если это лишниий Транкейт, то запишем с нулевым номером события
	//	Если СтрокиТекНаполнения.Количество() = 0 Тогда 
	//		
	//		НоваяСтрока = ДополнитьНаполнениеВТ(НастройкаТеста, СтрокаТаблицы, ИмяТаблицы, ДопТаблицы);
	//		НоваяСтрока.НомерСобытия = 0;
	//		СтрокиТекНаполнения.Добавить(НоваяСтрока);
	//		//ТекстОшибки = СтрШаблон("При обработке предыдущих наполнений ВТ %1 в сессии %2 почему то не заполнена таблица ТекущееНаполненияВТ, хотя есть данные ее начала - номер события %3, Дата события %4", 
	//		//	ИмяТаблицы, 
	//		//	СтрокаТаблицы.НомерСессии, 
	//		//	ДанныеНачала.НомерСобытияСоздания, 
	//		//	ДанныеНачала.Период);
	//		//НастройкаТестовСервер.ЗаписатьОшибкуВЖР(НастройкаТеста, ТекстОшибки, "НаполненияВременныхТаблицPostgres", ДопТаблицы);
	//		
	//	КонецЕсли;

	//	ЗаполнитьПоляНачалаИПеренестиНаполнениеВТ(ДанныеНачала, СтрокиТекНаполнения, ДопТаблицы);
	//	
	//КонецЕсли;
	//
	//ДанныеНачала.НомерСобытияСоздания = СтрокаТаблицы.НомерСобытия;
	//ДанныеНачала.Период = СтрокаТаблицы.ДатаСобытия;

КонецПроцедуры 

// Заполняем поля НомерСобытияСоздания,Период и переносим строки по текущей ВТ из ТЗ ТекущееНаполненияВТ в НаполненияВременныхТаблицPostgres 
Процедура ЗаполнитьПоляНачалаИПеренестиНаполнениеВТ(ДанныеНачала, СтрокиТекНаполнения = Неопределено, ДопТаблицы)
	
	Если ЗначениеЗаполнено(ДанныеНачала) Тогда
		
		Если СтрокиТекНаполнения = Неопределено Тогда
			СтруктураПоиска = Новый Структура("НомерСессии,ИмяТаблицы");
			СтруктураПоиска.НомерСессии = ДанныеНачала.НомерСессии;
			СтруктураПоиска.ИмяТаблицы = ДанныеНачала.ИмяТаблицы;
			
			СтрокиТекНаполнения = ДопТаблицы.ТекущееНаполненияВТ.НайтиСтроки(СтруктураПоиска);
		КонецЕсли;
		
		Для каждого Строка Из СтрокиТекНаполнения Цикл 
			
			НоваяСтрока = ДопТаблицы.НаполненияВременныхТаблицPostgres.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
			
			// Если это измененная строка, т.е. ее обработали при прошлом запуске, то выставяем признак ВидИзменения = 3 - для удаления, а заводим новую строку для записи
			Если НоваяСтрока.ВидИзменения = 2 Тогда
				НоваяСтрока.ВидИзменения = 3;
				НоваяСтрока = ДопТаблицы.НаполненияВременныхТаблицPostgres.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка,,"ВидИзменения");
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеНачала, "НомерСобытияСоздания,Период");
			
			Если ЗначениеЗаполнено(ДанныеНачала.Период) Тогда
				НоваяСтрока.ВидИзменения = 1;
				ДопТаблицы.ТекущееНаполненияВТ.Удалить(Строка);
			// Если это переход на следующую итерацию, то удалять не будем, а выставим признак Измененная
			Иначе
				Строка.ВидИзменения = 2;
			КонецЕсли;
...
// Добавляет строку в ТЗ ТекущееНаполненияВТ
Функция ДополнитьНаполнениеВТ(НастройкаТеста, СтрокаТаблицы, ИмяТаблицы, ДопТаблицы)

	//НоваяСтрока = ДопТаблицы.ТекущееНаполненияВТ.Добавить();
	//ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы, "НомерСессии,НомерСобытия");
	//НоваяСтрока.ИмяТаблицы 		= ИмяТаблицы;
	//НоваяСтрока.НастройкаТеста 	= НастройкаТеста;
	//НоваяСтрока.ВидИзменения 	= 1;
	//
...
Функция ИсправлениеОпределениеПодготовленногоОператораВТ(НастройкаТеста, ТаблицаРезультатов, СтрокаТаблицы, ТекстЗапросаMSSQL, ДопТаблицы) Экспорт 
	...
	//Если СтрокиВТ.Количество() = 0 Или СтрокиВТ[СтрокиВТ.ВГраница()].ОшибкаСоздания Тогда
	Если СтрокиВТ.Количество() = 0 Или СтрокиВТ[0].ДатаСоздания > СтрокаТаблицы.ДатаСобытия Тогда
		
		ПозНачПарам = СтрНайти(ТекстЗапросаMSSQL, ",n'");
		//ТекстПараметров = Сред(ТекстЗапросаMSSQL, ПозНачПарам + 5, ПозНач - ПозНачПарам - 6);
		...
		//ТипыПараметров = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТекстПараметров, ",", Истина);
		КолОткр = 0;
		...
		Для Сч = 0 По Поля.Вграница() Цикл
			...
			//Тип = Прав(ТипыПараметров[Сч], СтрДлина(ТипыПараметров[Сч]) - СтрДлина(XMLСтрока(Сч)) - 1);
		...
		// Исправлять пока не буду, т.к. есть проблема со строковыми типами Text и mvarchar
		//// Если есть ошибочная запись по созданию ВТ, то исправим ее
		//Если СтрокиВТ.Количество() > 0 И СтрокиВТ[СтрокиВТ.ВГраница()].ОшибкаСоздания Тогда
		//	
		//	ИсправитьСозданиеВТ(НастройкаТеста, ТаблицаРезультатов, ИмяТаблицы, СтрокиВТ[СтрокиВТ.ВГраница()], ДанныеПолейВременнойТаблицы, ДопТаблицы);	
		//	
		//КонецЕсли;
...
Функция ИсправлениеВызовПодготовленногоОператораВТ(НастройкаТеста, СтрокаТаблицы, ТекстЗапросаMSSQL, ДопТаблицы) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеТаблицы.ИмяТаблицы) Тогда
		...
		Для Сч = 0 По ЗначенияПараметров.ВГраница() Цикл
			...
			//Значение = НастройкаТестов_СозданиеЗапросов.ИсправитьЗначение(ЗначенияПараметров[Сч], ДанныеПолейВременнойТаблицы[Сч].Тип, Истина);
			Если ЗначенияПараметров[Сч] = "0x01" Тогда 
				
				Если ДанныеПолейВременнойТаблицы[Сч].Тип = "BOOLEAN" Тогда
					ЗначениеПараметра = "TRUE";
				Иначе 
					ЗначениеПараметра = "'\\001'::BYTEA";
				КонецЕсли;
		...
		ДополнитьНаполнениеВТ(НастройкаТеста, СтрокаТаблицы, ДанныеТаблицы.ИмяТаблицы, ДопТаблицы);
...
Процедура ИсправитьСозданиеВТ(НастройкаТеста, ТаблицаРезультатов, ИмяТаблицы, СтрокаВТ, ДанныеПолейВременнойТаблицы, ДопТаблицы)
	...
	Если Записи.Количество() = 0 Тогда
		...
		//ОшибочнаяЗапись.Измененная = Истина;
	Иначе
