...
Процедура ОбработатьСтрокиТаблицыРезультатов(НастройкаТеста, ТаблицаРезультатов, ДопТаблицы, ТекстСообщения, ЗаголовокСообщения = "") Экспорт 
	...
	Для каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
		...
		Если НЕ ДопТаблицы.Многопоточность И Сч % Шаг = 0 Тогда
			НастройкаТестовСервер.СообщитьПрогрессаЭтапа(ТекстСообщения, ЗаголовокСообщения, Окр(Сч/ТаблицаРезультатов.Количество()*10, 0) * 10);
		КонецЕсли;
		Сч = Сч + 1; 
	...
	Если КоличествоTOP > 1 Или ЕстьUnion И КоличествоTOP > 0 Тогда                
		...
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ПланЗапроса.ЗапросИсточник) Тогда
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ОшибкаПреобразования) Тогда
				НастройкаТестовСервер.УстановитьТекстОшибки(СтрокаТаблицы, "Поймали ситуацию, когда в процедуре в НастройкаТестов_СозданиеЗапросов.ИсправитьTOPВЗапросе перешли к разбору сложных TOP, но не заполнен СтрокаТаблицы.ПланЗапроса.ЗапросИсточник - массив подзапросов через union. Возможно недоразобрались параметры запроса rpc_completed, но нет ошибки""Ошибка разбора параметров"" или ""Ошибка получения плана запроса"". Нужна отладка");
			...
			Иначе	
				НастройкаТестовСервер.УстановитьТекстОшибки(СтрокаТаблицы, "Пропущен разбор сложных TOP в НастройкаТестов_СозданиеЗапросов.ИсправитьTOPВЗапросе");
			КонецЕсли;
...
Функция ИсправитьTOPВОтдельномЗапросе(ЗапросИсточник, СтрокаТаблицы, Знач Результат, КоличествоTOP, ОшибкиПреобразования, ПсевдонимРодителя = Неопределено)
	...
	Для каждого ПолеИсточник Из ЗапросИсточник Цикл
		...
		Если ПолеИсточник.ОсновнаяТаблица.Запрос <> Неопределено Тогда
			...
			Результат = ИсправитьTOPВОтдельномЗапросе(ПолеИсточник.ОсновнаяТаблица.Запрос.ЗапросИсточник, СтрокаТаблицы, Результат, КоличествоTOP, ОшибкиПреобразования, ПолеИсточник.ОсновнаяТаблица.Псевдоним);
...
Функция ИсправитьБинари1ВТексте(НастройкаТеста, СтрокаТаблицы, Знач Результат, ОшибкиПреобразования)
	...
	Если СтрНайтиПоРегулярномуВыражению(Результат, "\b0x01\b").Длина > 0 Тогда
		...
		Пока Истина Цикл
			...
			Для каждого РезультатПоиска Из РезультатыПоиска Цикл
				...
				Если СтрНайти(Группы[0].Значение, "::BYTEA") > 0 Или СтрНайти(Группы[2].Значение, "::BYTEA") > 0 Тогда
					Заменили = Истина; 
					Результат = СтрЗаменить(Результат, РезультатПоиска.Значение, СтрШаблон("%1%2%3", Группы[0].Значение, "'\\x01'::BYTEA", Группы[2].Значение));
					Прервать;
				КонецЕсли;	
			КонецЦикла;
		...
		Пока Истина Цикл
			...
			Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, Шаблон, "$1'\\\\x01'::BYTEA$5", Ложь, Ложь);
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\bCOALESCE\(\s*\(*[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\)*\s*,\s*\(*)0x01(\)*\s*\))", "$1'\\\\x01'::BYTEA$2", Ложь, Ложь); // COALESCE(  (t5._fld40406_type)  ,  (0x01)  ) 
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\bthen \(*[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\)* else \(*)0x01(\)* end)", "$1'\\\\x01'::BYTEA$2", Ложь, Ложь); // then (t1._q_000_f_005_type) else (0x01) end
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\bthen \(*)0x01(\)* else \(*\(*[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\)* end\b)", "$1'\\\\x01'::BYTEA$2", Ложь, Ложь); // then (0x01) else (t1._q_000_f_005_type) end
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "([a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\)*\s*)(=|<>|>|>=|<|<=|in)(\s*\(*)0x01\b", "$1$2$3'\\\\x01'::BYTEA", Ложь, Ложь); // Имя поля заканчивается на _type: T1._Q_000_F_016_type)) = ((0x01 или ._fld767_type <> 0x01 или snull(T2._Q_000_F_004_type,0x01)
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "((\bas\s+|::)BYTEA\)*(\s+end)?\s*(=|<>|>|>=|<|<=)\s*)0x01", "$1'\\\\x01'::BYTEA", Ложь, Ложь); // as BYTEA) end = 0x01 или and ('\\x03'::BYTEA >= 0x01) and (t3._fld1974 <
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\b(then|else) \(*'\\\\x[0-9a-f]{2}'::BYTEA\)* end,\s*)0x01(\))", "$1'\\\\x01'::BYTEA$3", Ложь, Ложь); // then '\\x08'::BYTEA end,0x01) - сложный запрос Внутри isnull идет case и заканчивается на эту подстрорку
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\bthen \(*)0x01(\)* (end else|else) \(*'\\\\x[0-9A-F]{2}'::BYTEA\)* end\b)", "$1'\\\\x01'::BYTEA$2", Ложь, Ложь); // then 0x01 else '\\x08'::BYTEA end
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\bthen \(*'\\\\x[0-9a-f]{2}'::BYTEA\)* (end else|else) \(*)0x01(\)* end\b)", "$1'\\\\x01'::BYTEA$3", Ложь, Ложь); // then '\\x08'::BYTEA else 0x01 end ИЛИ then '\\x08'::BYTEA end else 0x01 end
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "((then|else)\s+'\\\\x[0-9a-f]*'::BYTEA\s+end\)*\s*=\s*)0x01\b", "$1'\\\\x01'::BYTEA", Ложь, Ложь); // null then '\\x04'::BYTEA end = 0x01 
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(,\s*('\\\\(x[0-9a-f]{2}|001)'::BYTEA|[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type)\)+\s+(in|=|<>|>|>=|<|<=)\s+\(*)0x01", "$1'\\\\x01'::BYTEA", Ложь, Ложь); // COALESCE(t193._q_000_f_028_type,'\\x08'::BYTEA) in (0x01) либо любой байтишник, или поле заканчивающееся на _type
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "([a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\)*\s*,\s*)0x01\)", "$1'\\\\x01'::BYTEA)", Ложь, Ложь); // Имя поля заканчивается на _type: T1._Q_000_F_016_type, 0x01)
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "0x01(\)*\s*(=|<>|>|>=|<|<=)\s*\(*[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\b)", "'\\\\x01'::BYTEA$1", Ложь, Ложь); // 0x01 <> t1._q_001_f_022_type
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\b(else|then)\s+[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\s+end\s*=\s)0x01\b", "$1'\\\\x01'::BYTEA", Ложь, Ложь); // else t13._fld14585_type end = 0x01
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\b(then|else)\s+[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\s+end\s+else+\s+)0x01(\s+end\b)", "$1'\\\\x01'::BYTEA$3", Ложь, Ложь); // else t2._q_001_f_007_type end else 0x01 end
		...
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\b(else|then)\s+[a-z]\d{1,3}\.[a-z0-9_]{4,12}_type\s+end\)*\s+in\s+\()0x01\b", "$1'\\\\x01'::BYTEA", Ложь, Ложь); // else t82._q_000_f_005_type end in (0x01)
		
		// Должен быть ближе к концу
		Результат = СтрЗаменитьПоРегулярномуВыражению(Результат, "(\b(then|else)\s+'\\\\(x[0-9a-f]+|001)'::BYTEA\s+(end\s+){1,}(in|=|<>|>|>=|<|<=)\s*\(*)0x01\b", "$1'\\\\x01'::BYTEA", Ложь, Ложь); // else '\\x01'::BYTEA end in (0x01)
		...
		Для каждого Строка0x01 Из Строки0x01 Цикл
			...
			Если Строка0x01.НомерCase >= 0 Тогда 
				...
				Если Заменили Тогда
					...
					НоваяСтрока.ПодстрокаЗамены = СтрЗаменитьПоРегулярномуВыражению(ПодстрокаПоиска, НастройкаТестовКлиентСервер.Шаблон0x01вCase(), ?(ЭтоБулево,"$1TRUE", "$1'\\\\x01'::BYTEA"));
...
Функция ЗаменитьПараметрыВЗапросе(СтрокаТаблицы, Знач ТекстЗапроса, ДопТаблицы)
	...
	Если СтрНайти(ТекстЗапроса, "exec sp_executesql n'") > 0 Тогда
		...
		Если ЗначенияПараметров.Количество() <> Парам.Количество() Тогда
			...
			НастройкаТестов_СозданиеЗапросов.УстановитьОшибкуПреобразования(ДопТаблицы.ОшибкиПреобразования, СтрокаТаблицы, "ОшибкаРазбораПараметров",,ДопТаблицы.Многопоточность);
...
Процедура УстановитьОшибкуПреобразования(ОшибкиПреобразования, СтрокаТаблицы, ФлагДобавления = "", ФлагИсключения = "", Многопоточность = Ложь) Экспорт 
	...
	Если Строки.Количество() = 0 Тогда
		...
		Если Многопоточность Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ВызватьИсключение("Ошибка записи ошибки преобразования " + ОписаниеОшибки());
			КонецПопытки;
...
Процедура ЗаполнитьПланЗапроса(СтрокаТаблицы, Результат, ДопТаблицы)
	...
	Если ЗаполняемПланЗапроса Тогда
		...
		ПланЗапроса = ПланЗапросов.ОбъектЗапроса(Результат, СтрокаТаблицы.СтроковыеПараметры);
		Если ТипЗнч(ПланЗапроса) = Тип("Структура") И ПланЗапроса.Свойство("ЕстьОшибка") И ПланЗапроса.ЕстьОшибка Тогда
			
			НастройкаТестовСервер.УстановитьТекстОшибки(СтрокаТаблицы, ПланЗапроса.ПолноеОписаниеОшибки);
			НастройкаТестов_СозданиеЗапросов.УстановитьОшибкуПреобразования(ДопТаблицы.ОшибкиПреобразования, СтрокаТаблицы, "ОшибкаПолученияПланаЗапроса",,ДопТаблицы.Многопоточность);
			СтрокаТаблицы.ПланЗапроса = Неопределено;
			
		Иначе	                      
			
			СтрокаТаблицы.ПланЗапроса = ПланЗапроса;
...
Функция ИсправитьОтдельныеБинари1ВОбычномЗапросе(СтрокаТаблицы, Знач Результат, ДопТаблицы) Экспорт 
	...
	Если КоличествоОтдельноСтроящих0x01 > 0 И ЕстьUnion И ЗначениеЗаполнено(СтрокаТаблицы.ПланЗапроса) Тогда
		
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ПланЗапроса.ЗапросИсточник) Тогда
			
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ОшибкаПреобразования) Тогда
				НастройкаТестовСервер.УстановитьТекстОшибки(СтрокаТаблицы, "Поймали ситуацию, когда в процедуре в НастройкаТестов_СозданиеЗапросов.ИсправитьОтдельныеБинари1ВОбычномЗапросе перешли к разбору отдельностоящих 0x01, но не заполнен СтрокаТаблицы.ПланЗапроса.ЗапросИсточник - массив подзапросов через union. Возможно недоразобрались параметры запроса rpc_completed, но нет ошибки ""Ошибка разбора параметров"" или ""Ошибка получения плана запроса"". Нужна отладка");
				НастройкаТестов_СозданиеЗапросов.УстановитьОшибкуПреобразования(ДопТаблицы.ОшибкиПреобразования, СтрокаТаблицы, ?(СтрокаТаблицы.ВидСобытия = Перечисления.ВидыСобытийЗапросов.ЗаполнениеВременнойТаблицы, "ОтладкаСозданияВТ", "ОтладкаСозданияЗапросов"),,ДопТаблицы.Многопоточность);
			Иначе	
				НастройкаТестовСервер.УстановитьТекстОшибки(СтрокаТаблицы, "Пропущен разбор отдельностоящих 0x01 в подзапросах через Union в НастройкаТестов_СозданиеЗапросов.ИсправитьОтдельныеБинари1ВОбычномЗапросе");
			КонецЕсли;
	...
	Если Есть0x01ВCase > 0 И ЕстьUnion И ЗначениеЗаполнено(СтрокаТаблицы.ПланЗапроса) Тогда
		
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ПланЗапроса.ЗапросИсточник) Тогда
			
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.ОшибкаПреобразования) Тогда
				НастройкаТестовСервер.УстановитьТекстОшибки(СтрокаТаблицы, "Поймали ситуацию, когда в процедуре в НастройкаТестов_СозданиеЗапросов.ИсправитьОтдельныеБинари1ВОбычномЗапросе перешли к разбору case имеющих отдельностоящие 0x01 в подзапросах через Union, но не заполнен СтрокаТаблицы.ПланЗапроса.ЗапросИсточник - массив подзапросов через union. Возможно недоразобрались параметры запроса rpc_completed, но нет ошибки""Ошибка разбора параметров"" или ""Ошибка получения плана запроса"". Нужна отладка");
				НастройкаТестов_СозданиеЗапросов.УстановитьОшибкуПреобразования(ДопТаблицы.ОшибкиПреобразования, СтрокаТаблицы, ?(СтрокаТаблицы.ВидСобытия = Перечисления.ВидыСобытийЗапросов.ЗаполнениеВременнойТаблицы, "ОтладкаСозданияВТ", "ОтладкаСозданияЗапросов"),,ДопТаблицы.Многопоточность);
			Иначе	
				НастройкаТестовСервер.УстановитьТекстОшибки(СтрокаТаблицы, "Пропущен разбор case имеющих отдельностоящие 0x01 в подзапросах через Union в НастройкаТестов_СозданиеЗапросов.ИсправитьОтдельныеБинари1ВОбычномЗапросе");
			КонецЕсли;
	...
	Для каждого ПолеИсточникаБис Из ЗапросИсточник Цикл
		...
		Иначе 
			...
			Если ЗначениеЗаполнено(ТаблицаЗамен) Тогда
				...
				ПодстрокаЗамены = ?(ТаблицаЗамен[0].ЭтоБулево, "TRUE", "'\\x01'::BYTEA");				
				Прервать;
