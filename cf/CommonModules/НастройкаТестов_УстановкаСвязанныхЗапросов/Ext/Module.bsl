...
Функция ПроверитьСвязьСОшибкамиВТ(СтрокаТаблицы, ДопТаблицы, РегШаблон = "\bpg_temp\.(tt\d+)\b") Экспорт 
...
Процедура ВыполнитьОдинПоток(НастройкаТеста, ТаблицаРезультатов = Неопределено, ДопТаблицы) Экспорт	
	...
	Для каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
		...
		Если НЕ ДопТаблицы.Многопоточность И Сч % Шаг = 0 Тогда
			НастройкаТестовСервер.СообщитьПрогрессаЭтапа(ДопТаблицы.ТекстСообщения, ДопТаблицы.ЗаголовокСообщения, Окр(Сч/ТаблицаРезультатов.Количество()*10, 0) * 10);
		КонецЕсли;
		Сч = Сч + 1; 
		...
		Если НЕ ПроверитьСвязьСОшибкамиВТ(СтрокаТаблицы, ДопТаблицы, "\bpg_temp\.(tt\d+)\b") И ЗначениеЗаполнено(СтрокаТаблицы.ОшибкаПреобразования) Тогда
			НастройкаТестов_СозданиеЗапросов.УстановитьОшибкуПреобразования(ДопТаблицы.ОшибкиПреобразования, СтрокаТаблицы, ,"СвязанныйЗапрос",ДопТаблицы.Многопоточность);	
		КонецЕсли;
