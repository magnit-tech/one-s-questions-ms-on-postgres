...
Процедура ЗагрузитьДанныеФон(Параметры, АдресХранилища) Экспорт 
	...
	Результат = ЗагрузитьДанные(Параметры.НастройкаТеста, Параметры.Вариант, Параметры.ДанныеФайлов, Параметры.ОчиститьЭтап, Параметры.ПараметрыТеста);
	ПоместитьВоВременноеХранилище(Результат, АдресХранилища);

КонецПроцедуры

Функция ЗагрузитьДанные(НастройкаТеста, Вариант, ДанныеФайлов, ОчиститьЭтап = Ложь, ПараметрыТеста = Неопределено, ДобавитьЧасыКДатеПриЗагрузке = 0) Экспорт 
...
Процедура ЗагрузкаИзБазы(НастройкаТеста, ДопПараметры)
	...
	Пока Истина Цикл
		...
		Сч = 0;
		ТекстСообщения = "Обработка и загрузка..."; 
		...
		//	
		//	Если Сч % Шаг = 0 Тогда
		//		НастройкаТестовСервер.СообщитьПрогрессаЭтапа(ТекстСообщения, ЗаголовокСообщения, Окр(Сч/Выборка.Количество()*100, 0));
		//	КонецЕсли;
		//	Сч = Сч + 1;
		//	
...
Функция ДанныеПозицийXML()
	...
	ПодстрокиИсключения.Добавить("FROM dbo._ExtensionsInfoNGS ");
...
Функция РезультатИзБазы(НастройкаТеста, ПараметрыТеста, ЭтапОбработки, ЗаголовокСообщения, МаксID)
	...
	Если ПараметрыТеста.ПорцияЗагрузкиДанных = 0 Тогда 
		...
		ТекстСообщения = "Чтение всех данных из базы расширенных событий";
	...
	Иначе
		...
		ТекстСообщения = СтрШаблон("Чтение данных из базы расширенных событий %1 записей", ПараметрыТеста.ПорцияЗагрузкиДанных);
		
	КонецЕсли;
	
	НастройкаТестовСервер.СообщитьПрогрессаЭтапа(ТекстСообщения, ЗаголовокСообщения);
