	...
	Попытка
		...
		Пока Истина Цикл
			...
			Если ПараметрыТеста.ИнтервалОбработкиДанных = 0 Тогда 
				...
				ТекстСообещения = "Из всех записей...";
			Иначе
...
Процедура ОбработатьСтрокиТаблицыРезультатов(НастройкаТеста, ТаблицаРезультатов, ДопТаблицы, ЗаголовокСообщения = "", ТекстСообещения)
    Перем ДатаНач;
	...
	Для каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
		...
		Если Сч % Шаг = 0 Тогда
			НастройкаТестовСервер.СообщитьПрогрессаЭтапа(ЗаголовокСообщения, ТекстСообещения, Окр(Сч/ТаблицаРезультатов.Количество()*100, 0));
		КонецЕсли;
		Сч = Сч + 1;
		...
		Если 	СтрокаТаблицы.ВидСобытия = ПредопределенноеЗначение("Перечисление.ВидыСобытийЗапросов.СозданиеВременнойТаблицы") 
			...
			Если СтрокаТаблицы.ВидСобытия = ПредопределенноеЗначение("Перечисление.ВидыСобытийЗапросов.СозданиеВременнойТаблицы") Тогда 
				...
				Если ПредыдущаяСтрока <> Неопределено И ПредыдущаяСтрока.Создана Тогда
					...
					КонецЕсли;
					//ТекстЗапроса = СтрШаблон("DROP TABLE pg_temp.%1_%2", ИмяТаблицы, Формат(СтрокаТаблицы.НомерСессии, "ЧГ=0"));
		...
		Если СтрокаТаблицы.ВидСобытия = ПредопределенноеЗначение("Перечисление.ВидыСобытийЗапросов.ВыборкаДанных") Тогда
			...
			// Возможна ошибка комобъекта при подсчете Выборка.RecordCount:
			// Ошибка при получении значения атрибута контекста (RecordCount):
			// по причине:
...
Функция ПолучитьСоединение(НомерСессии, ДопТаблицы,Команда) Экспорт 
	...
	Если Соединение = Неопределено Тогда
        ...
        Соединение.ConnectionString = ДопТаблицы.ПараметрыТеста.СтрокаПодключенияКPostgres;
			...
			//"server="+ИмяСервераPostgres+";"+
			//"Port="+ПортPostgres+";"+
			//"uid="+ПользовательPostgres+";"+
			//"pwd="+ПарольPostgres+";"+
			//"database="+БазаДанныхPostgres+";";
        Соединение.ConnectionTimeout = 30;
		//Если ЗначениеЗаполнено(ДопТаблицы.ПараметрыТеста.КоэффициентОжиданияВыполненияЗапросовНаPostgres) Тогда
		//    Соединение.CommandTimeout = ДопТаблицы.ПараметрыТеста.ТаймаутЗапросовНаPostgres;
		//Иначе			
		...
		//Если ЗначениеЗаполнено(ДопТаблицы.ПараметрыТеста.ТаймаутЗапросовНаPostgres) Тогда
		//	Запросы.Добавить(СтрШаблон("SET statement_timeout to %1", Формат(ДопТаблицы.ПараметрыТеста.ТаймаутЗапросовНаPostgres * 1000, "ЧГ=0"))); // Этого не было, но иначе ошибка Произошла исключительная ситуация (Microsoft OLE DB Provider for ODBC Drivers): ERROR: canceling statement due to statement timeout;
		//КонецЕсли;
...
Процедура ВыполнитьОдинПоток(НастройкаТеста, ТаблицаРезультатов = Неопределено, ДопТаблицы) Экспорт	
	...
	ОбработатьСтрокиТаблицыРезультатов(НастройкаТеста, ТаблицаРезультатов, ДопТаблицы, ДопТаблицы.ЗаголовокСообщения, ДопТаблицы.ТекстСообещения);
