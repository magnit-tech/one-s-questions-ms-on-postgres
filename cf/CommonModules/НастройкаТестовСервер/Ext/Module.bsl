...
Процедура УдалитьДанныеРегистровНастройкиТеста(НастройкаТеста, Вариант = 1) Экспорт 
	...
	Если Вариант = 1 Или Вариант = 4 Тогда	
		...
		//Массив.Добавить("ПодготовленныеОператоры");
		//Массив.Добавить("ПараметрыПодготовленныхОператоров");
...
Функция УдалениеДанныхСОтбором(НастройкаТеста, Настройки) Экспорт 
	...
	// Не получилось вручную инициализировать СКД, не понимаю, почему не заполняются поля в наборе, созданном вручную, поэтому пока закоментил и использую макет
	...
	//|	ВидСобытия,
	//|	ТекстЗапросаMSSQL,
	//|	КоличествоСтрокMSSQL,
	//|	ДлительностьВыполненияMSSQL,
	//|	ТекстЗапросаPostgres,
	//|	КоличествоСтрокPostgres,
	//|	ДлительностьВыполненияPostgres,
	//|	ВидОшибки	
	//|	ТекстОшибки
	//|ИЗ
	//|  	РегистрСведений.РезультатыВыполненияЗапросов 
...
Процедура СообщитьПрогрессаЭтапа(ЗаголовокСообщения = "", ТекстСообщения, Процент = Неопределено) Экспорт 
...
Функция ТаблицаРезультатов(НастройкаТеста, ДопТаблицы) Экспорт 
	...
	//Если ДопТаблицы.Свойство("ЗаголовокСообщения") Тогда
	//	НастройкаТестовСервер.СообщитьПрогрессаЭтапа(ДопТаблицы.ЗаголовокСообщения, "Подготовка данных");
	//КонецЕсли;
	...
	//|	ПроизвольнаяИнформация КАК ПроизвольнаяИнформация
	|ИЗ
	...
	//Таблица.ЗаполнитьЗначения(Перечисления.ВидыОшибокЗапросов.ПустаяСсылка(), "ВидОшибки"); 
...
Функция СессииПоПотокамТаблицыРезультатов(НастройкаТеста, ДопТаблицы) Экспорт 
	...
	// Нельзя - массив соединений не сериализуется
	//// Для выполненения запросов сразу откроем соединения, иначе увеличатся ошибки ОшибочноеИспользованиеВТ - т.к. не будут учитываться созданные и наполненные ВТ в предыдущих итерациях
	//Если ДопТаблицы.ЭтапОбработки.ЭтапОбработки = Перечисления.ВидыЭтаповОбработки.ВыполнениеЗапросов Тогда
	//	Для каждого КиЗ Из Потоки Цикл
	//		Для каждого НомерСессии Из КиЗ.Значение.НомераСессий Цикл
	//			НастройкаТестов_ВыполнениеЗапросов.ПолучитьСоединение(НомерСессии, ДопТаблицы);	
	//		КонецЦикла;	
	//	КонецЦикла;		
	//КонецЕсли;
...
Функция СоздатьЗапуститьПотоки(НастройкаТеста, Результат, ДопТаблицы) Экспорт 
	...
	СообщитьПрогрессаЭтапа(ДопТаблицы.ЗаголовокСообщения, "Разбивка по потокам"); 
	...
	Для каждого КиЗ Из Потоки Цикл
       ...
       // определяем массив параметров для процедуры
		...
		//ПараметрыВыполнения.ЗапуститьВФоне = НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая();
		
		// запуск фонового задания
		//Задание = ФоновыеЗадания.Выполнить(ДопТаблицы.Метод, НаборПараметров);
		...
		ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
			//ПредставлениеОшибки = Новый Структура("КраткоеПредставлениеОшибки,ПодробноеПредставлениеОшибки");
			//ЗаполнитьЗначенияСвойств(ПредставлениеОшибки, ДлительнаяОперация);
...
Процедура ВыполнитьИтерациюЭтапа(НастройкаТеста, ДопТаблицы, Результат) Экспорт 
	...
	Иначе
		...
		СообщитьПрогрессаЭтапа(ДопТаблицы.ЗаголовокСообщения, "Подготовка данных");
		...
		//ВыполнитьОдинПоток(НастройкаТеста,ТаблицаРезультатов,,ДопТаблицы);
...
Функция ТаблицаРезультатовПоЗапросу(Запрос, НастройкаТеста, ДопТаблицы)
	...
	Если ДопТаблицы.ЭтапОбработки <> Неопределено Тогда
		...
		Если ДопТаблицы.Отборы.ПараметрыТеста.ИнтервалОбработкиДанных > 0 Тогда 
			...
			//Если НЕ ЗначениеЗаполнено(Отборы.ЭтапОбработки.ТекущаяДатаСобытия) Тогда
			//	
			//	Отборы.ЭтапОбработки.ТекущаяДатаСобытия = ДатаМинимум-1;
			//	
			//КонецЕсли;
...
Процедура ДобавитьКолонкиИзмененияВТаблицу(Таблица, ВидИзменения = 0) Экспорт 
	...
	//// Если нужно учесть, то это данные из регистра, то выставим вид изменения - он будет скорее всего 2
	//Если ЗначениеЗаполнено(ВидИзменения) Тогда
	//	Таблица.ЗаполнитьЗначения(ВидИзменения, "ВидИзменения");
	//КонецЕсли;
