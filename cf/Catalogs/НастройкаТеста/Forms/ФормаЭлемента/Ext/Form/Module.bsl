...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если ОтборВТХранилище <> Неопределено Тогда
		...
		НастройкиКомпоновкиДанных = ОтборВТХранилище.Получить();
		НастройкиКомпоновкиДанных.ПараметрыДанных.Элементы[0].Значение = Объект.Ссылка;
		СписокВременныеТаблицыPostgres.КомпоновщикНастроек.ЗагрузитьФиксированныеНастройки(НастройкиКомпоновкиДанных);
...
Асинх Процедура ПередатьФайлыНаСервер(Вариант = 1) 
	...
	Иначе
		...
		Для каждого Описание Из Результат Цикл
			...
			КонецЕсли;
			ДанныеФайла = Новый Структура("Адрес,Имя,Расширение", Описание.Адрес, Описание.СсылкаНаФайл.Имя, Описание.СсылкаНаФайл.Расширение);
...
Процедура ЗагрузитьОтборНаСервере(ИмяКоманды) 
	...
	// Переопределим параметр на ссылку
	НастройкиКомпоновкиДанных = СтрокаТаблицы.Настройка.Получить();
	НастройкиКомпоновкиДанных.ПараметрыДанных.Элементы[0].Значение = Объект.Ссылка;
	РезультатыВыполненияЗапросов.КомпоновщикНастроек.ЗагрузитьФиксированныеНастройки(НастройкиКомпоновкиДанных);
	Элементы.РезультатыВыполненияЗапросов.Обновить();
...
Функция ДанныеИсправленныхСозданийВТ()
	...
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Справочник.НастройкаТеста КАК Настройка
	|ПО
	|	Настройка.Ссылка = РС.НастройкаТеста
	|ГДЕ
	|   НЕ Настройка.ПометкаУдаления
	|СГРУППИРОВАТЬ ПО
	| 	КолЗаписей.КоличествоЗаписей,
	|	РС.НастройкаТеста
	|УПОРЯДОЧИТЬ ПО
...
&НаКлиенте
Асинх Процедура СозданиеЗапросовНаКлиенте()
...
&НаКлиенте
Процедура СсылкаНаСобытиеВOpenSearch(Команда)
	
	НастройкаТестовКлиент.СкопироватьВБуферСсылкуНаСобытиеВOpensearch(Объект.Ссылка, Элементы.РезультатыВыполненияЗапросов.ТекущиеДанные);
